...
//	
	...
	// cksohn - xl o2p 기능 추가
	private byte dbType = XLCons.PPAS;

	
	public  XLPPASConnection(String _dbIp, String _dbSid, String _userId, String _passwd, int _port, byte _dbType){
		this.dbIp = _dbIp;
		this.dbSid = _dbSid;
		this.userId = _userId;
		this.passwd = _passwd;
	...
	private void createConnection() {		
		...
		for ( int i = 0; i < XLConf.XL_DBCON_RETRYCNT; i++ ) {	
			try {
				...
				XLLogger.outputInfoLog(this.dbType+"/"+this.dbIp+"/"+this.port+"/"+this.userId+"/"+this.passwd+"/"+this.dbSid+"/");
