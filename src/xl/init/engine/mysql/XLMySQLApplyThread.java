...
public class XLMySQLApplyThread extends Thread {
...
//	private XLOracleConnection oraConnObj = null;
	// gssg - xl o2m 지원
//	private XLMariaDBConnection mySQLConnObj = null; // gssg - xl m2m 기능 추가  - 0413
...
//	private OraclePreparedStatement pstmtInsert = null;
	...
	public XLMySQLApplyThread(XLJobRunPol jobRunPol) {
		super();
	...
	public void run(){
		...
		try {
			...
			this.pstmtInsert = (PreparedStatement)mySQLConnObj.getConnection().prepareStatement(this.jobRunPol.getTarInsertSql());
			...
			while( true )
			{
				if ( XLInit.STOP_FLAG ) {
					...
					this.mySQLConnObj.rollback(); // gssg - xl m2m 기능 추가  - 0413
...
//				}
				...
				if ( vtData.size() == 1) {
					...
					if ( recordDataArray == null ) {
						...
						this.mySQLConnObj.commit(); // gssg - xl m2m 기능 추가  - 0413
