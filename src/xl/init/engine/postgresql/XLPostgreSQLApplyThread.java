...
public class XLPostgreSQLApplyThread extends Thread {
	...
	private XLPostgreSQLConnection postgreSQLConnObj = null; // cksohn - xl o2p 기능 추가
	...
	public XLPostgreSQLApplyThread(XLJobRunPol jobRunPol) {
		super();
	...
	public void run(){
		...
		try {
			...
			postgreSQLConnObj = new XLPostgreSQLConnection( // cksohn - xl o2p 기능 추가
					tdbInfo.getIp(), 
					tdbInfo.getDbSid(),
					tdbInfo.getUserId(),
					tdbInfo.getPasswd(),
			...
			// cksohn - xl o2p 기능 추가
			this.pstmtInsert = (PreparedStatement)postgreSQLConnObj.getConnection().prepareStatement(this.jobRunPol.getTarInsertSql());
			...
			while( true )
			{
				if ( XLInit.STOP_FLAG ) {
					...
					this.postgreSQLConnObj.rollback(); // cksohn - xl o2p 기능 추가
...
//				}
				...
				if ( vtData.size() == 1) {
					...
					if ( recordDataArray == null ) {
						...
						this.postgreSQLConnObj.commit(); // cksohn - xl o2p 기능 추가
...
//							}
						...
						case XLDicInfoCons.XMLTYPE:
							...
							if ( colInfo.getFunctionStr().equalsIgnoreCase("XL_REPLACE_NULL") ) {
								value = removeNullChar(value);
								
								if (value != null & !value.equals("") ) {
									byte[] byteArr = XLUtil.hexToByteArray(value);
									
								    if ( this.jobRunPol.getTdbInfo().getCharSet().contains("WIN949") || this.jobRunPol.getTdbInfo().getCharSet().contains("CP949")) {
								    	value = new String (byteArr, "MS949");
								    } else if ( this.jobRunPol.getTdbInfo().getCharSet().contains("EUC") ) {
								    	value = new String (byteArr, "EUC-KR");
								    } else if ( this.jobRunPol.getTdbInfo().getCharSet().contains("KSC5601") ) {
								    	value = new String (byteArr, "KSC5601");
								    } else {
						...
						case XLDicInfoCons.NCLOB:
							if ( colInfo.getFunctionStr().equalsIgnoreCase("XL_REPLACE_NULL") ) {
								...
								if (value != null & !value.equals("") ) {
								    ...
								    if ( this.jobRunPol.getTdbInfo().getnCharSet().contains("WIN949") || this.jobRunPol.getTdbInfo().getnCharSet().contains("CP949")) {
								    	value = new String (byteArr, "MS949");
								    } else if ( this.jobRunPol.getTdbInfo().getnCharSet().contains("EUC") ) {
								    	value = new String (byteArr, "EUC-KR");
								    } else if ( this.jobRunPol.getTdbInfo().getnCharSet().contains("KSC5601") ) {
								    	value = new String (byteArr, "KSC5601");
								    } else {
								    	value = new String (byteArr, "UTF-8");
								    }			
	...
	// gssg - SK텔레콤 O2M, O2P
    public String removeNullChar(String hexString) {
        StringBuilder result = new StringBuilder();

        // 2문자씩 잘라서 확인
        for (int i = 0; i < hexString.length(); i += 2) {
            String byteString = hexString.substring(i, i + 2);
            if (!byteString.equals("00")) {
