					...
					case XLDicInfoCons.BLOB:
						...
						// binary 타입 (hex-->binary)
							...
							if ( bAry != null && (tdbType == XLCons.ORACLE 
									|| tdbType == XLCons.TIBERO) ) { // 오라클과 티베로는 바이너리 지원 안됨
								value = null;
							} 
							// gssg - xl 전체적으로 보완2
							// gssg - PostgreSQL 커넥터 처리
...
//				 XLLogger.outputInfoLog("CKSOHN DEBUG sb_record = " + sb_record.toString());
				...
				// cksohn - BULK mode oracle sqlldr
				// cksohn - xl bulk mode 성능 개선 - t2o - 건단위로 write 하도록
			...
			// cksohn - xl bulk mode 성능 개선 - t2o - comment - 건단위로 write  하면 아래 부분 불필요
