...
public class XLTiberoRecvThread extends Thread {
	...
	public XLTiberoRecvThread(XLJobRunPol jobRunPol) {
		super();
	...
	public void run(){
		...
		try {
			...
			XLLogger.outputInfoLog("DEBUG getSrcSelectSql = " +  this.jobRunPol.getSrcSelectSql());
					...
					case XLDicInfoCons.XMLTYPE:
						...
						if ( XLConf.XL_LOB_STREAM_YN ) { // cksohn - xl XL_LOB_STREAM_YN=Y|*N
							...
							if ( value != null && colInfo.getFunctionStr().equalsIgnoreCase("XL_REPLACE_NULL") ) {			
								...
								byte[] hexBytes = value.getBytes(StandardCharsets.UTF_8);
								
								value = XLUtil.bytesToHexString(hexBytes);
//								XLLogger.outputInfoLog("################################################################");
//								XLLogger.outputInfoLog("[GSSG DEBUG] value = " + value);
//								XLLogger.outputInfoLog("################################################################");									
							}
